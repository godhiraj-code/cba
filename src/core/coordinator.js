...
class Coordinator extends EventEmitter {
    ...
    pruneIntentHistory(now = Date.now()) {
        ...
        while (this.intentHistory.size >= this.maxIntentHistory) {
                ...
                try {
                    ...
                    if (outcome.status === 'failed') {
                        const error = new ProtocolError(
                            ...
                            { intentId: intent.id, sentinel: selected, evidence: outcome.evidence, attempts: history }
                        );
                        this.emit('intent.failed', { intent, error });
                        throw error;
                    }
